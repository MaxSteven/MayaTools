...
class sp3dObjectList (object):
	...
	def addObj(self, obj=None, dagPath=None, activation=True, proba=0.5, align='Up'):
		...
		if (self.alreadyExists(obj)):
			...
			objchild = mc.listRelatives(obj, children=True, type='mesh') #get all the children shapes of the object
			print objchild
...
def getDAGPath(node, depth=False):
	...
	if nodetype == 'transform':
		...
		childlist = mc.listRelatives(node, children=True, type='mesh')
		if childlist:
			if len(childlist) == 1:
				# there's only 1 shape below the transform
				if depth:
					dag = mc.listRelatives(node, fullPath=True, type='mesh')
				else:
					dag = mc.listRelatives(node, path=True, type='mesh')
