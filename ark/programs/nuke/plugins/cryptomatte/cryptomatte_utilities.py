...
def setup_cryptomatte_ui():
    if nuke.GUI:
        ...
        automatte_menu.addCommand("Cryptomatte", "import cryptomatte_utilities as cu; cu.cryptomatte_create_gizmo();", "shift+c")
        automatte_menu.addCommand("Decryptomatte All", "import cryptomatte_utilities as cu; cu.decryptomatte_all();")
        automatte_menu.addCommand("Decryptomatte Selection", "import cryptomatte_utilities as cu; cu.decryptomatte_selected();", "shift+d")
...
def cryptomatte_create_gizmo():
    node = nuke.createNode("Cryptomatte")
    return node
...
#    start_dot = nuke.nodes.Dot(inputs=[gizmo])
    expr_node = nuke.nodes.Expression(
        channel0='rgba', expr0=expr_setting,
        name="CryptExpr_%s"%matte_name, disable=disabled_setting
    )

    # set crypto expression location
    expr_node.setXYpos(gizmo.xpos(), gizmo.ypos() + 30)
...
#        shufflecopy.knob("alpha").setValue("red")
    ...
    gizmo.knob("disable").setValue(False)
