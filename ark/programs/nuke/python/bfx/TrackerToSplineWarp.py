...
def trackerToRoto(tracker, framerange):
	global cancel

	task = nuke.ProgressTask( 'Tracker to Splinewarp Pins' )
	tracks = tracker['tracks']
	tracksScript = tracks.toScript()
	trackScriptParts = tracksScript.split(' ')
	numTracks = int(trackScriptParts[3])


	# splineWarp = nuke.nodes.SplineWarp3(name='warp_' + tracker['name'].getValue(),
	# 									xpos=(tracker.xpos() + 220),
	# 									ypos=tracker.ypos())

	rotoNode = nuke.nodes.Roto(name='roto_' + tracker['name'].getValue(),
										xpos=(tracker.xpos() + 220),
										ypos=tracker.ypos())
	# warpCurve = splineWarp['curves']
	# warpRoot = warpCurve.rootLayer

	# roto = nuke.nodes.SplineWarp3(xpos=(tracker.xpos() + 220), ypos=tracker.ypos() + 100)
	...
	for n in range(numTracks):
		...
		if task.isCancelled():
			cancel = True
			break
		task.setMessage('%d of %d' % (n, numTracks))
		task.setProgress(int(float(n)/numTracks)*100)

		shape = rp.Shape(rotoCurve, type='bspline')
		point = rp.ShapeControlPoint(0,0)

		xIndex = n * nCols + 2
		yIndex = n * nCols + 3
		for frame in framerange:
			x = tracks.getValueAt(frame, xIndex)
			y = tracks.getValueAt(frame, yIndex)
			point.center.addPositionKey(frame, (x, y))

		shape.append(point)
		rotoRoot.append(shape)
...
def main():
	...
	try:
		trackerNode = nuke.selectedNode()
		if trackerNode.Class() != 'Tracker4':
			if nuke.GUI:
				nuke.message('Unsupported node type. Selected Node must be Tracker')
			return
	except:
		if nuke.GUI:
			nuke.message('Unsupported node type. Selected Node must be Tracker')
	...
	if result == 0:
		return # Canceled
	try:
		framerange = nuke.FrameRange(p.knobs()["framerange"].getText())
	except:
		if nuke.GUI:
	...
	if nuke.NUKE_VERSION_MAJOR < 7:
		nuke.message('This script only works in Nuke 7')
		return


	# real work starts here
	trackerNode = nuke.selectedNode()
	rotoNode = trackerToRoto(trackerNode, framerange)

	rptsw_shapeList = []
	nukescripts.node_copypaste()
	warpNode = nuke.createNode('SplineWarp3')
	warpNode.setName('Warp_' + trackerNode.name())
	warpCurve = warpNode['curves']
	warpRoot = warpCurve.rootLayer
	rotoCurve = rotoNode['curves']
	rotoRoot = rotoCurve.rootLayer
	...
	breakshapesintoPin(rotoNode, framerange)
	rptsw_shapeList = []
	rptsw_shapeList = rptsw_walker(rotoRoot, rptsw_shapeList)
	...
	for shape in rptsw_shapeList:
		if isinstance(shape[0], nuke.rotopaint.Shape):
			if multi:
				...
				threading.Thread(None, bakeShapes, args=(shape, warpNode, framerange, rotoRoot, rptsw_shapeList, task)).start()
			else:
				bakeShapes(shape, warpNode,framerange, rotoRoot, rptsw_shapeList,task)
		n+=1
	#===========================================================================
