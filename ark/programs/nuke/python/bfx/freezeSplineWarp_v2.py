import nuke, time, threading, nuke.splinewarp as sw, _curveknob as ck
    ...
    if nuke.NUKE_VERSION_MAJOR > 6:
        ...
        if exp:
            ...
            for shape in shapeList:
                ...
                if mt:
                    thread = threading.Thread(None,expressionLock, args=(shape,ab,freezeFrame,node,task))
            ...
            for shape in shapeList:
                ...
                task.setProgress((int(n/len(shapeList)*100)))
                if mt:
                    thread = threading.Thread(None,keyFreeze, args=(shape,ab,freezeFrame,task))
                    threads.append(thread)
                    thread.start()
                else:
