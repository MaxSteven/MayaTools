...
# modo.exe "-cmd:@C:/ie/ark/tools/objScreenshot/objThumbnail.py <filename> <destname> JPG"
...
# Destname does not need an extension; the JPG extension currently given as argument will be appended to destname
# The final argument can be any format that Modo supports rendering in
...
def makeScreenShot(fileName, imagePath, fileFormat='JPG'):
	...
	posnVector = [bbox[0][i]+2*deltas[i] for i in range(3)]
	...
	#Set render options
	lx.eval('render.res axis:1 res:384')
	lx.eval('render.res axis:0 res:384')
	scene.select('Render')

	cmd = 'item.channel outPat <none>'
	lx.eval(cmd)
	for material in scene.items('advancedMaterial'):
		material.channel('diffAmt').set(1.0)
		material.channel('specAmt').set(1.0)
		material.channel('diffCol.R').set(0.3)
		material.channel('diffCol.G').set(0.3)
		material.channel('diffCol.B').set(0.3)
		material.channel('specCol.R').set(0.2)
		material.channel('specCol.G').set(0.2)
		material.channel('specCol.B').set(0.2)
