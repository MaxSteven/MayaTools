...
try:
    ...
    '''
    Replace HIP env variables, very unfortunate
    ...
    try:
        ...
        jobID = re.match(r'.*/jobsData/(\w+)/\w+.hip', inputFile)
        if jobID:
            jobID = jobID.group(1)
            env = ad.getJob(jobID)['Props'].get('Env')
            print(env)
            for k, v in env.items():
                hou.hscript('setenv ' + k + ' = ' + v)
    except Exception as err:
        traceback.print_exc(err)
        print('Could not set job environment')
    ''' end '''
    ...
    if rop == None:
        ...
        if ropType == 'rop_geometry':
            ...
            # Make sure render to Mplay is turned off since we don't want it to show on the screen but we want it to render an output image.
            renderToMPlay = rop.parm( "HO_renderToMPlay" )
            if renderToMPlay is not None:
                renderToMPlay.set( 0 )

            renderToFile = rop.parm( "HO_img_enable" )
            if renderToFile is not None:
                renderToFile.set( 1 )

            renderCreateIntermediate = rop.parm( "HO_img_createDir" )
            if renderCreateIntermediate is not None:
                renderCreateIntermediate.set( 1 )

            if output is not None:
                outputFile = rop.parm( "HO_img_fileName" )
                if outputFile is not None:
                    outputFile.set( output )
