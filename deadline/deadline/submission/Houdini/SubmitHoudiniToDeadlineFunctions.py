...
def GetOutputPath( node ):
    ...
    elif nodeType == "customGeo":
        outputFile = node.parm("cacheNodePath").evalAsNode().parm("filename")
...
def GetFrameInfo( renderNode ):
    ...
    elif renderNode.type().description() == "customGeo":
        print( "+++++++++++++++xxxxxxxxxxxxxxxxxxxxxxxxx++++++++++++++++++" )
        print('CUSTOM GEO FOUND WITH PATH', renderNode.parm("cacheNodePath").evalAsNode())
        print('+++++++++++++++xxxxxxxxxxxxxxxxxxxxxxxxx++++++++++++++++++')
        if renderNode.parm("cacheNodePath").evalAsNode():
            print('{} is a cacheNode with evaluated node {}'.format(renderNode, renderNode.parm("cacheNodePath").evalAsNode()))
            return GetFrameInfo(renderNode.parm("cacheNodePath").evalAsNode())
        ...
        if localExport:
            ...
            for regionjobNum in range( 0, regionJobCount ):
                ...
                with open( jobInfoFile, "w" ) as fileHandle:
                    ...
                    # IE_customGeo node submission for simulation
                    # Author: Shobhit Khinvasara
                    # Date : 4/4/2018
                    # '''
                    elif node.type().name() == 'ie_customgeo':
                        if node.parm('cacheNodePath').evalAsNode().parm('is_sim').eval():
                            startFrame = node.parm('cacheNodePath').evalAsNode().parm('f1').eval()
                            endFrame = node.parm('cacheNodePath').evalAsNode().parm('f2').eval()
                            frameLength = int(endFrame - startFrame) + 1
                            fileHandle.write( "ChunkSize=%s\n" % str(frameLength))
                        else:
                            fileHandle.write( "ChunkSize=%s\n" %  jobProperties.get( "framespertask", 1 ) )
                    # '''
